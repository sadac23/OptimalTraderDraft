        ...
        using (StreamWriter writer = new StreamWriter(alertFilePath))
        {
            ...
            foreach (Analyzer.AnalysisResult r in results)
            {
                if (r.ShouldAlert())
                {
                    ...
                    writer.WriteLine($"株価：{r.StockInfo.LatestPrice.ToString("N1")}（{r.StockInfo.LatestPriceDate.ToString("yyyy/MM/dd")}：L{r.StockInfo.LatestPriceRSIL.ToString("N2")},S{r.StockInfo.LatestPriceRSIS.ToString("N2")}）{(r.StockInfo.OversoldIndicator() ? mark : string.Empty)}{(r.StockInfo.IsOwnedNow() && r.StockInfo.OverboughtIndicator() ? mark : string.Empty)}");
